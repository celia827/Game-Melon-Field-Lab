...
let mission=null,missionCharge=0,gravityUntil=0,missionTimer=0;
...
function syncUI(){
  $('score').textContent=score;$('best').textContent=best;$('energy').value=energy;$('energy-value').textContent=Math.floor(energy);$('stir').disabled=energy<30||ended||paused;
  if(mission){$('mission-kind').textContent=mission.done?'已完成':'进行中';$('mission-text').textContent=mission.text;$('mission-progress').max=mission.target;$('mission-progress').value=Math.min(mission.progress,mission.target);$('mission-value').textContent=`${Math.min(mission.progress,mission.target)} / ${mission.target}`;}
  $('lab-charge').textContent=`${missionCharge} / 3`;const ready=missionCharge>=3&&!gravityUntil;$('gravity-button').disabled=!ready||ended||paused;$('gravity-status').textContent=gravityUntil?`${Math.ceil(Math.max(0,gravityUntil-world.time))} 秒`:(ready?'可使用':'充能中');
  $('game').dataset.fruits=world.bodies.length;$('game').dataset.phase=ended?'ended':paused?'paused':'playing';
}
...
function newMission(){const choices=[{kind:'合成目标',text:'合成一个柑橘',target:1,check:e=>e.type===2},{kind:'物理实验',text:'在果汁模式下完成 2 次融合',target:2,check:e=>world.mode==='juice'},{kind:'连锁实验',text:'连续完成 2 次融合',target:2,check:e=>combo>=2}];return {...choices[Math.floor(Math.random()*choices.length)],progress:0,done:false};}
function completeMission(){if(!mission||mission.done)return;mission.done=true;score+=100;energy=Math.min(100,energy+15);missionCharge=Math.min(3,missionCharge+1);if(score>best){best=score;try{localStorage.setItem('melon-lab-best',String(best));}catch{}}toast('委托完成！实验瓶充能 +1');tone(620,.16);syncUI();missionTimer=window.setTimeout(()=>{if(!ended){mission=newMission();syncUI();toast('新的实验委托已到达。');}},1400);}
const world=new World(event=>{
  ...
  if(mission&&!mission.done&&mission.check(event)){mission.progress++;if(mission.progress>=mission.target)completeMission();}
...
function reset(){world.reset();world.mode=document.querySelector('[data-mode][aria-pressed=true]').dataset.mode;score=0;energy=100;current=0;next=0;cooldown=0;ended=false;hasWon=false;paused=false;effects=[];ripples=[];combo=0;lastMerge=-10;mission=newMission();missionCharge=0;gravityUntil=0;if(missionTimer)clearTimeout(missionTimer);missionTimer=0;keys.clear();heldTilt=0;if(dialog.open)dialog.close();updateNext();resume();$('aim-tip').textContent='移动瞄准 · 点击投放';}
...
function useGravity(){if(!loaded||paused||ended||missionCharge<3||gravityUntil)return;missionCharge=0;gravityUntil=world.time+8;world.gravityScale=.5;toast('减重剂生效，果池变轻了。');tone(520,.22);syncUI();}
...
$('restart').addEventListener('click',()=>{if(score===0){reset();return;}showDialog('开始一池新的水果？','本局分数将清零，最佳分数会保留。','重新开始',reset);});
 $('help').addEventListener('click',()=>showDialog('一点物理，一点好运','<p>移动鼠标瞄准，点击投放。手机上拖动瞄准，松手投放。</p><p>同类水果碰到一起就会融合。切换「果冻 / 半流体 / 果汁」，感受不同弹性与流动性。</p><p>按 A / D 倾斜果池，按空格搅动。搅动消耗 30 能量；合成与等待可恢复。</p><p>完成实验委托可获得额外分数和能量。实验瓶充满 3 格后，使用减重剂让重力减半 8 秒。</p><p>水果稳定越过虚线 3 秒，本局结束。合出西瓜后可以继续挑战。</p>','开始实验',resume));
